...
# Hallo, {{ $data->buyer_name }}

Terima kasih sudah memesan layanan terapi di {{ config('app.name') }}.
...
<span class="tbr_display-block tbr_text-center tbr_mb-1">Total yang harus dibayar adalah :</span>
<span class="tbr_display-block tbr_text-center tbr_text-bold tbr_text-danger tbr_text-h2 tbr_mb-2">{{ formatPrice($data->transaction_amount) }}</span>

@if ($data->buyer_payment_method === "Transfer")
<span class="tbr_text-medium">Pembayaran dapat di transfer melalui :</span> <br>
{{ $setting->bank_name }} Nomor Rekening {{ $setting->account_number }} a.n. {{ $setting->bank_account }}

<span class="tbr_text-medium">Catatan :</span> <br>
Pastikan nilai total tagihan Anda sudah benar kepada {{ $setting->bank_account }}.
Jika ada pertanyaan terkait invoice ini, silakan hubungi
62 81393108310. Apabila Anda sudah melakukan Transfer,
silakan kirim Bukti Transfer kepada Admin.
@elseif($data->buyer_payment_method === "Cash")
<span class="tbr_text-medium">Bayar Cash</span> <br>
Silakan bayar sesuai dengan nominal di atas secara langsung
kepada terapis {{ $data->therapist->name }}.
@endif


@component('mail::button', ['url' => route('patient.order.detail', $data->id)])
