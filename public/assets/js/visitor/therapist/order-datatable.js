...
let notifIcon = `
    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" style="position: relative; left: 15px" class="datatable-notifIcon-order">
...
function initDatatables(url, orderFalse, callback = "") {
    return $(".table").DataTable({
        ...
        rowCallback: function (row, data, nColumn) {
            if (data[5].replace(/(<([^>]+)>)/gi, "") == "Menunggu") {
                $("td", row).css("background-color", "#FDF7F8");
                $('td:eq(0)', row).find(".datatable-notifIcon-order").remove();
                $('td:eq(0)', row).append(notifIcon);
            }
        },
