...
function initDatatables(url, orderFalse, callback = "") {
	return $(".table").DataTable({
		...
		ajax: {
			...
			error: function (res) {
				...
				if (typeof res.responseJSON.errors === "object") {
					...
					errorList += "Terjadi kesalahan pada sistem. Silakan reload ulang halaman ini.";
				}
				new PNotify({
					title: "Gagal!",
					text: errorList,
		...
		drawCallback: function (data) {
			$("#total-transaction").html(data.json.recordsFiltered);
			$("#total-therapist-fee").html(data.json.totalCommission)
